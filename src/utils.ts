...
export class BilibiliParser {
    ...
    private async fetchTags(bvid: string): Promise<string[]> {
        try {
            const response: any = await this.ctx.http.get('https://api.bilibili.com/x/tag/archive/tags', {
                params: { bvid },
                headers: { 'User-Agent': this.config.userAgent },
            });
            if (response?.code !== 0) return [];
            return (response.data || []).map((tag: any) => tag.tag_name).filter(Boolean);
        } catch (error) {
    ...
    private async fetchOnline(aid: number, cid: number): Promise<string | null> {
        try {
            const response: any = await this.ctx.http.get('https://api.bilibili.com/x/player/online/total', {
                params: { aid, cid },
                headers: { 'User-Agent': this.config.userAgent },
            });
    ...
    private async renderVideoCard(info: any): Promise<Buffer> {
        ...
        try {
            await page.setContent(this.generateCardHtml(info, danmaku, comments, tags, online), { waitUntil: 'networkidle2' });
    ...
    private generateCardHtml(info: any, danmaku: string[], comments: any[], tags: string[], online: string | null): string {
...
<style>
  * { box-sizing: border-box; }
  ...
  img.emoji { display: inline-block; width: 1em; height: 1em; margin: 0 .05em; vertical-align: -.12em; }
  ...
  .online svg { width: 16px; height: 16px; fill: currentColor; }
...
<main class="card">
  ...
  <div class="main">
    ...
    <div class="owner"><img src="${this.escapeHtml(owner.face)}" /><div><div class="owner-name">${this.escapeHtml(owner.name)}</div><div class="publish-date">${this.formatDate(info.pubdate)} 发布</div></div></div>
    <div class="desc">${this.escapeHtml(info.desc || '暂无简介')}</div>
    ${danmakuHtml}
    ${commentsHtml}
  </div>
</main>
<script>if (window.twemoji) twemoji.parse(document.body, { base: 'https://cdn.jsdelivr.net/gh/jdecked/twemoji@v17.0.3/assets/', folder: 'svg', ext: '.svg' });</script>
</body></html>`;
    }
