...
import { Renderer, extractResourceUrls, type ImageSource, type Node } from '@takumi-rs/core';
import type { Config } from './index';

// Takumi helper 的子路径类型使用 .mts，当前 CommonJS 项目的 Node 解析模式无法读取，但运行时导出正常。
const htmlHelpers: { fromHtml(html: string): { node: Node; stylesheets: string[] } } = require('@takumi-rs/helpers/html');
const emojiHelpers: { extractEmojis(node: Node, type: 'twemoji'): Node } = require('@takumi-rs/helpers/emoji');
...
export class BilibiliParser {
    ...
    private async renderVideoCard(info: any): Promise<Buffer> {
        ...
        const fetchedResources = await this.fetchCardResources(extractResourceUrls(node));
    ...
    private generateCardHtml(info: any, danmaku: string[], comments: any[], tags: string[], online: string | null, fontFamily: string): string {
